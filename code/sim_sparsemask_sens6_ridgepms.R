...
#Oct 14, change corr of pms to 0.6, and don't do cv on ridge
...
for (iter in 1:n_iterations) {
  for (a in signal_types) {
    ...
    for(q in (1:length(noise.strs))){
      ...
      rho_high <- rep(-log(0.6)/4,length=ncol(sub.dat))
      ...
      results <- rbind(results, data.frame(SignalType = a, Method = "pms0.6", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      fit.ridge <- glmnet(sub.dat.s,res.var, alpha=0, lambda = 1e-3)
      beta <- coef(fit.ridge)
      ...
      rho_high <- rep(-log(0.6)/4,length=ncol(sub.dat.s))
      #pms high
      ...
      metrics <- accuracy_cal(rank.vec = out.pms.hi, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "pms_smooth", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      metrics <- accuracy_cal(rank.vec = out.holp, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "holp_smooth", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
    }
  }
}

write.csv(results, '/well/nichols/users/qcv214/pms2/pile/sim_oct14_sparsemask_ridgepms.csv', row.names = FALSE)
