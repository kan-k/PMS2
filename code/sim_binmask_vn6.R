...
for (iter in n_iterations) {
  for (a in signal_types) {
    ...
    for(q in (1:length(noise.strs))){
      ...
      # Store the results
      results <- rbind(results, data.frame(SignalType = a,
                                           Method = "ridge", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "PCA90", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "SPCA90", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      results <- rbind(results, data.frame(SignalType = a, Method = "pms", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      metrics <- accuracy_cal(rank.vec = out.holp, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "holp", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      var.sel <- (order(abs(beta_no_int), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "ridge_smooth", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "PCA90_smooth", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "PCA90_smooth_proj", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "PCA90_smooth_cov", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "SPCA90_smooth", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "SPCA90_smooth_proj", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "SPCA90_smooth_cov", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
      ...
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "tmap", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR, sample_size = sample.size))
...
write.csv(results,paste0('/well/nichols/users/qcv214/pms2/pile/sim_oct4_v300n_binmask_iter',n_iterations,'.csv'), row.names = FALSE)