...
mcd_result_t <- covMcd(t(X_weighted_sel))
mcd_cov_t <- mcd_result_t$cov
# Perform eigen decomposition on the covariance matrix
# mcd_cov <- (1/(nrow(X_weighted_sel)-1)) * mcd_cov_t
mcd_cov <- nrow(X_weighted_sel) * mcd_cov_t
...
write.csv(out,paste0( '/well/nichols/users/qcv214/pms2/pile/june9_cv_pms_ridgeSMCDPCA_600.csv'), row.names = FALSE)
