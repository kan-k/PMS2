...
# library(rrcov)
#Sep 26, use true beta derived from `gen_age_HOLP_sparsemask.R` and data, as assessed, gen from U[0,1]
  ...
  for (a in signal_types) {
    ...
    for(q in (1:length(noise.strs))){
      ...
      results <- rbind(results, data.frame(SignalType = a,
                                           Method = "ridge", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "PCA90", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "SPCA90", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      results <- rbind(results, data.frame(SignalType = a, Method = "pms", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      metrics <- accuracy_cal(rank.vec = out.holp, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "holp", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      var.sel <- (order(abs(beta_no_int), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "ridge_smooth", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "PCA90_smooth", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "PCA90_smooth_proj", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "PCA90_smooth_cov", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "SPCA90_smooth", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "SPCA90_smooth_proj", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      var.sel <- (order(abs(beta_pms), decreasing=TRUE))
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "SPCA90_smooth_cov", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      ...
      metrics <- accuracy_cal(rank.vec = var.sel, mask.signal = mask.signal)
      results <- rbind(results, data.frame(SignalType = a, Method = "tmap", SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
...
write.csv(results, '/well/nichols/users/qcv214/pms2/pile/sim_oct3_sparsemask.csv', row.names = FALSE)
