...
for (iter in 1:n_iterations) {
  for (a in signal_types) {
    ...
    for(q in (1:length(noise.strs))){
      ...
      for(rho in rhos){
        print(rho)
        rho_high <- rep(-log(rho)/4,length=ncol(sub.dat))
        ...
        results <- rbind(results, data.frame(SignalType = a, Method = paste0("pms",rho), SignalStrength = rsquare, Iteration = iter,Overlap = metrics$overlap,FPR = metrics$FPR,FDR = metrics$FDR))
      }
...
write.csv(results, '/well/nichols/users/qcv214/pms2/pile/sim_oct16_pms_grid.csv', row.names = FALSE)
