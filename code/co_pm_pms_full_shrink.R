...
# format(object.size(sub.dat), units = "Mb")
print("subdat.test")

colnames(sub.dat.test) <- column.names

# format(object.size(sub.dat.test), units = "Mb")
...
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_ridge'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_ridge_rank'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_PCA'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_PCA_rank'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_PCA_proj'))
var.sel <- (order(abs(beta_pms), decreasing=TRUE))
mask.temp <-oro.nifti::readNIfTI('/well/nichols/users/qcv214/bnn2/res3/res3mask.nii.gz')
mask.temp[mask.temp!=0][var.sel] <- c(ncol(sub.dat):1)/ncol(sub.dat)
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_PCA_proj_rank'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_PCA_cov'))
var.sel <- (order(abs(beta_pms), decreasing=TRUE))
mask.temp <-oro.nifti::readNIfTI('/well/nichols/users/qcv214/bnn2/res3/res3mask.nii.gz')
mask.temp[mask.temp!=0][var.sel] <- c(ncol(sub.dat):1)/ncol(sub.dat)
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_PCA_cov_rank'))
########################################
####Removing previous data
remove(lambda.80)
remove(pre_beta_pms)
...
# pca.cv.90 <- simple.cv(lambda.80) #theta = 10 is optimal
# print("rmse pca 90")
# res.lambda.pca.90.cor <- long.rmse.with.lambda.theta.ridge(lambda.80, pca.cv.90[1,which.min(pca.cv.90[3,])], num.vox.vec)
# 
# #Plotting 
# Omeg <- solve(sub.dat%*%lambda.80%*%t(sub.dat) + (pca.cv.90[1,which.min(pca.cv.90[3,])])*diag(nrow(sub.dat))) #4x4
# pre_beta_pms <- lambda.80%*%t(sub.dat)%*%Omeg #4 x 75
...
# mask.temp@datatype = 16
# mask.temp@bitpix = 32
# writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_PCA90_corr'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_SPCA'))
var.sel <- (order(abs(beta_pms), decreasing=TRUE))
mask.temp <-oro.nifti::readNIfTI('/well/nichols/users/qcv214/bnn2/res3/res3mask.nii.gz')
mask.temp[mask.temp!=0][var.sel] <- c(ncol(sub.dat):1)/ncol(sub.dat)
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_SPCA_rank'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_SPCA_proj'))
var.sel <- (order(abs(beta_pms), decreasing=TRUE))
mask.temp <-oro.nifti::readNIfTI('/well/nichols/users/qcv214/bnn2/res3/res3mask.nii.gz')
mask.temp[mask.temp!=0][var.sel] <- c(ncol(sub.dat):1)/ncol(sub.dat)
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_SPCA_proj_rank'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_SPCA_cov'))
var.sel <- (order(abs(beta_pms), decreasing=TRUE))
mask.temp <-oro.nifti::readNIfTI('/well/nichols/users/qcv214/bnn2/res3/res3mask.nii.gz')
mask.temp[mask.temp!=0][var.sel] <- c(ncol(sub.dat):1)/ncol(sub.dat)
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_SPCA_cov_rank'))
...
remove(Omeg)
########################## corr
# pca<- prcomp(X_weighted, center = TRUE, scale. = TRUE)
# explained_variance <- pca$sdev^2 / sum(pca$sdev^2)
# # Calculate the cumulative variance explained
# cumulative_variance <- cumsum(explained_variance)
# # Determine the number of components needed to explain at least 90% of the variance
...
# spca.cv.90 <- simple.cv(lambda.80) #theta = 10 is optimal
# print("rmse SPCA 90")
# res.lambda.spca.90.cor <- long.rmse.with.lambda.theta.ridge(lambda.80, spca.cv.90[1,which.min(spca.cv.90[3,])], num.vox.vec)
# 
# #Plotting 
# Omeg <- solve(sub.dat%*%lambda.80%*%t(sub.dat) + (spca.cv.90[1,which.min(spca.cv.90[3,])])*diag(nrow(sub.dat))) #4x4
# pre_beta_pms <- lambda.80%*%t(sub.dat)%*%Omeg #4 x 75
...
# mask.temp@datatype = 16
# mask.temp@bitpix = 32
# writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/sep24_pm_dcv_smooth_SPCA90_corr'))
...
write.csv(out,paste0( '/well/nichols/users/qcv214/pms2/pile/sep24_pm_dcv_pms90_smooth_shrink.csv'), row.names = FALSE)
