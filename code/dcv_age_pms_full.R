...
long.rmse <- function(ranked_coef, num.vox.vec){
  ...
  for(i in num.vox.vec){
    ...
    fit.ridge.pca <- cv.glmnet(sub.dat.s[,var.sel],age_tab$age, alpha=0) #fixing lambda
...
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug30_age_dcv_smooth_ridge'))
...
long.rmse.with.lambda.theta.ridge <- function(lambda,theta,num.vox.vec){
  ...
  for(i in num.vox.vec){
    ...
    #ridge prediction
    # fit.ridge <- cv.glmnet(sub.dat.s[,var.sel],age_tab$age, alpha=0)
    fit.ridge <- cv.glmnet(sub.dat.s[,var.sel],age_tab$age, alpha=0) #fixing lambda
    # beta <- coef(fit.ridge, s = "lambda.min")
    beta <- coef(fit.ridge, s = "lambda.min")
    train <- c(train,sqrt(mean((as.numeric(age_tab$age - cbind(1,sub.dat.s[,var.sel])%*%beta))^2)))
    test <- c(test,sqrt(mean((as.numeric(age_tab.test$age - cbind(1,sub.dat.s.test[,var.sel])%*%beta))^2)))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug30_age_dcv_smooth_PCA90'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug30_age_dcv_smooth_SPCA90'))
...
write.csv(out,paste0( '/well/nichols/users/qcv214/pms2/pile/aug30_age_dcv_pms8090_full_smooth.csv'), row.names = FALSE)
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug30_age_dcv_smooth2_ridge'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug30_age_dcv_smooth2_PCA90'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug30_age_dcv_smooth2_SPCA90'))
...
out <- rbind(res.lambda.pca.90$train,res.lambda.spca.90$train ,res.ridge$train, 
             res.lambda.pca.90$test,res.lambda.spca.90$test,res.ridge$test)
write.csv(out,paste0( '/well/nichols/users/qcv214/pms2/pile/aug30_age_dcv_pms8090_full_smooth2.csv'), row.names = FALSE)
