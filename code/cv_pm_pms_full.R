...
long.rmse <- function(ranked_coef, num.vox.vec){
  ...
  for(i in num.vox.vec){
    ...
    fit.ridge.pca <- glmnet(sub.dat.s[,var.sel],age_tab$age, alpha=0, lambda = 1e-2) #fixing lambda
    beta <- coef(fit.ridge.pca)
    train <- c(train,sqrt(mean((as.numeric(age_tab$age - cbind(1,sub.dat.s[,var.sel])%*%beta))^2)))
    test <- c(test,sqrt(mean((as.numeric(age_tab.test$age - cbind(1,sub.dat.s.test[,var.sel])%*%beta))^2)))
  }
  out <- list()
  out$train <- train
  out$test <- test
...
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug29_pm_smooth_ridge'))
...
simple.cv <- function(lambda, no.variable = 1000,theta.range. = theta.range){ #Changed from -9 to -8, to -7 for 2k
  ...
  for(theta in theta.range.){
    ...
    print(5)
    fit.ridge <- cv.glmnet(sub.dat.s[,var.sel],age_tab$age, alpha=0)
...
long.rmse.with.lambda.theta.ridge <- function(lambda,theta,num.vox.vec){
  ...
  for(i in num.vox.vec){
    ...
    #ridge prediction
    # fit.ridge <- cv.glmnet(sub.dat.s[,var.sel],age_tab$age, alpha=0)
    fit.ridge <- glmnet(sub.dat.s[,var.sel],age_tab$age, alpha=0, lambda = 1e-2) #fixing lambda
    # beta <- coef(fit.ridge, s = "lambda.min")
    beta <- coef(fit.ridge)
    train <- c(train,sqrt(mean((as.numeric(age_tab$age - cbind(1,sub.dat.s[,var.sel])%*%beta))^2)))
    test <- c(test,sqrt(mean((as.numeric(age_tab.test$age - cbind(1,sub.dat.s.test[,var.sel])%*%beta))^2)))
...
# pca <- PcaHubert(sub.dat.s,kmax=100) 
print("rmse robpca")

print("tune pca")
pca <-  prcomp(sub.dat.s)
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug29_pm_smooth_PCA90'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug29_pm_smooth_SPCA90'))
...
write.csv(out,paste0( '/well/nichols/users/qcv214/pms2/pile/aug29_pm_cv_pms8090_full_smooth.csv'), row.names = FALSE)
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug29_pm_smooth2_ridge'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug29_pm_smooth2_PCA90'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug29_pm_smooth2_SPCA90'))
...
out <- rbind(res.lambda.pca.90$train,res.lambda.spca.90$train ,res.ridge$train, 
             res.lambda.pca.90$test,res.lambda.spca.90$test,res.ridge$test)
write.csv(out,paste0( '/well/nichols/users/qcv214/pms2/pile/aug29_pm_cv_pms8090_full_smooth2.csv'), row.names = FALSE)
...
out <- rbind(res.lambda.pca.90$train,res.lambda.spca.90$train ,res.ridge$train, 
             res.lambda.pca.90$test,res.lambda.spca.90$test,res.ridge$test)
write.csv(out,paste0( '/well/nichols/users/qcv214/pms2/pile/aug29_pm_cv_pms8090_full.csv'), row.names = FALSE)
...
write.csv(out,paste0( '/well/nichols/users/qcv214/pms2/pile/aug29_pm_nbpms_full.csv'), row.names = FALSE)
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug29_pm_pms_hi'))
...
mask.temp@datatype = 16
mask.temp@bitpix = 32
writeNIfTI(mask.temp,paste0('/well/nichols/users/qcv214/pms2/viz/aug29_pm_pms_holp'))
