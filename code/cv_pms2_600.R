...
part_use<-part_use[1:600,] #only take 200
...
out <- rbind(res.lambda.ropca$train,res.lambda.pca$train,res.ridge$train,res.lambda.pls$train,res.lambda.spca$train,
             res.lambda.ropca$test,res.lambda.pca$test,res.ridge$test,res.lambda.pls$test,res.lambda.spca$test)
...
write.csv(out,paste0( '/well/nichols/users/qcv214/pms2/pile/may27_cv_pms_600.csv'), row.names = FALSE)
