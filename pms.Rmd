...
for (signal_type in signal_types) {
  ...
  p <- ggplot(signal_data, aes(x = SignalStrength, y = Overlap_Median, color = Method)) +
    ...
    scale_color_manual(values = custom_colors) +  # Use custom colors
    theme_minimal() +
    labs(title = paste("Median Power vs R-square -", signal_type),
         x = "R-square",
         y = "Median Power",
         color = "Method") +
    theme(
...
for (signal_type in signal_types) {
  ...
  p <- ggplot(signal_data, aes(x = SignalStrength, y = Overlap_SD, color = Method)) +
    ...
    scale_color_manual(values = custom_colors) +  # Use custom colors
    theme_minimal() +
    labs(title = paste("s.d. of Power vs R-square -", signal_type),
         x = "R-square",
         y = "s.d. Power",
         color = "Method") +
    theme(
...
for (signal_strength in signal_strengths) {
  ...
  p <- ggplot(signal_data, aes(x = sample_size, y = Overlap_Median, color = Method)) +
    ...
    scale_color_manual(values = custom_colors) +  # Use custom colors
    theme_minimal() +
    labs(title = paste("Median Power vs No. Samples - with R^2 = ", signal_strength),
         x = "No. Samples",
         y = "Median Power",
         color = "Method") +
    theme(
...
#Bin mask
mask.signal <- which(mask.index)[1:300]
sqrt(sum(mask.beta[mask.signal]^2))
var(mask.beta[mask.signal])
#dbmask
mask.signal <- which(mask.index)[c(1:150,1001:1150)]
sqrt(sum(mask.beta[mask.signal]^2))
var(mask.beta[mask.signal])
...
created `sim_binmask_sens6_smooth.R` to assess the smoothing parameter
[i think it ran out of time] `sim_oct11_binmask_smooth.csv` 16495521 => smoothing = 0.25, 0.5, 1, 2
sim_oct11_sparsemask_smooth.csv  16895739 => 3 runs
sim_oct11_dbmask_smooth.csv. 16895751

`sim_oct11_binmask_smooth.csv` 16890423 => reduce number it to 3, also keep track of the progress
...
##Look at varying smoothing param
...
signal_types <- c("binmask")#, "dbmask", "sparsemask")
# signal_types <- c("binmask")
base_path <- "/well/nichols/users/qcv214/pms2/pile/sim_oct11_"
...
for (signal_type in signal_types) {
  # Load the CSV file
  file_path <- paste0(base_path, signal_type, "_smooth.csv")
  results <- read_csv(file_path)
...
New way of showing smoothing
```{r}
# Base color scheme for methods
base_colors <- c(
  "PCA90_smooth" = "#7570b3",
  "SPCA90_smooth" = "#e7298a",
  "ridge_smooth" = "#77868e",
...
smoothing_params <- c(0.25, 0.5, 1, 2)

# Generate new custom_colors dynamically for each smoothing magnitude
custom_colors <- list()

for (method in names(base_colors)) {
  for (smooth_val in smoothing_params) {
    # Correct naming convention for method with smoothing parameter
    method_with_suffix <- gsub("_smooth", paste0("_smooth", smooth_val), method)
    custom_colors[[method_with_suffix]] <- base_colors[[method]]
  }
}

# Convert the custom_colors list to a named vector for ggplot
custom_colors <- unlist(custom_colors)

# Unique Signal Types
signal_types <- unique(results$SignalType)

# Loop through each smoothing parameter and create a separate plot
for (smooth_val in smoothing_params) {
  
  for (signal_type in signal_types) {
    # Filter the results for the current SignalType and Smoothing Parameter
    signal_data <- subset(results, SignalType == signal_type & grepl(paste0("_smooth", smooth_val), Method))
    
    # Create the plot with bolder lines and distinct colors
    p <- ggplot(signal_data, aes(x = SignalStrength, y = Overlap_Median, color = Method)) +
      geom_line(size = 1.5) +  # Make the lines bolder with size = 1.5
      ylim(0,80) +
      scale_color_manual(values = custom_colors) +  # Use dynamic colors
      theme_minimal() +
      labs(title = paste("Median Power vs R-square -", signal_type, "- Smoothing:", smooth_val),
           x = "R-square",
           y = "Median Power",
           color = "Method") +
      theme(
...
# Define the signal types and the file path template
signal_types <- c("dbmask")#, "dbmask", "sparsemask")
# signal_types <- c("binmask")
base_path <- "/well/nichols/users/qcv214/pms2/pile/sim_oct11_"
...
# Define the signal types and the file path template
signal_types <- c("sparsemask")#, "dbmask", "sparsemask")
# signal_types <- c("binmask")
base_path <- "/well/nichols/users/qcv214/pms2/pile/sim_oct11_"
...
Running
//sim_oct11_binmask_smooth_pms.csv  17168777
//sim_oct11_sparsemask_smooth_pms.csv 17168771

Also change corr of pms to 0.6, and don't do cv on ridge
//sim_oct14_binmask_ridgepms.csv 17168887
//sim_oct14_sparsemask_ridgepms.csv 17169193

#14 Oct

##Smooth params for pms
###bin mask
```{r}
# Load necessary packages
if (!require("pacman")) {install.packages("pacman"); library(pacman)}
...
for (signal_type in signal_types) {
  # Load the CSV file
  file_path <- paste0(base_path, signal_type, "_smooth_pms.csv")
  results <- read_csv(file_path)
...
```
Seems like HOLP is at least as good as PMS


##Look at sensitivity for smooth holp
sim_oct14_binmask_ridgepms.csv
```{r}
# Load necessary packages
if (!require("pacman")) {install.packages("pacman"); library(pacman)}
p_load(dplyr, readr)
...
# Define the signal types and the file path template
signal_types <- c("binmask", "sparsemask")
# signal_types <- c("binmask")
base_path <- "/well/nichols/users/qcv214/pms2/pile/sim_oct14_"
...
for (signal_type in signal_types) {
  # Load the CSV file
  file_path <- paste0(base_path, signal_type, "_ridgepms.csv")
  results <- read_csv(file_path)
...
```
PMS is best here. For sparse mask, see how pms0.6 is almost same as holp_smooth

Let's combine these results with the old ones (replace) oct3

#15 Oct

##Simulation study - sensitivity
combine new ridge result (no cv) with lower pms
...
# Initialize an empty list to store the aggregated and filtered results for each signal_type
all_aggregated_results_filtered <- list()
...
for (signal_type in signal_types) {
  ...
  # Filter the unwanted methods ("ridge", "ridge_smooth", "pms")
  aggregated_results_filtered <- aggregated_results %>%
    filter(!(Method %in% c("ridge", "ridge_smooth", "pms")))
  
  # Append the filtered results to the list
  all_aggregated_results_filtered[[signal_type]] <- aggregated_results_filtered
}

# Concatenate all filtered aggregated results into one matrix
results <- bind_rows(all_aggregated_results_filtered)
```
```{r}
# Load necessary packages
if (!require("pacman")) {install.packages("pacman"); library(pacman)}
...
# Define the signal types and the file path templates
signal_types <- c("binmask", "sparsemask")
base_path_oct3 <- "/well/nichols/users/qcv214/pms2/pile/sim_oct3_"
base_path_oct14 <- "/well/nichols/users/qcv214/pms2/pile/sim_oct14_"

# Initialize empty lists to store the aggregated results for each signal_type
all_aggregated_results_oct3 <- list()
all_aggregated_results_oct14 <- list()

# Loop through each signal_type for the first base path (Oct 3)
for (signal_type in signal_types) {
  # Load the CSV file
  file_path <- paste0(base_path_oct3, signal_type, ".csv")
  results_oct3 <- read_csv(file_path)
  
  # Aggregate the results by Method and SignalStrength
  aggregated_results_oct3 <- results_oct3 %>%
    group_by(Method, SignalStrength) %>%
    summarise(
      Overlap_Median = median(Overlap, na.rm = TRUE),
      Overlap_SD = sd(Overlap, na.rm = TRUE),
  ...
  # Filter unwanted methods ("ridge", "ridge_smooth", "pms")
  aggregated_results_oct3_filtered <- aggregated_results_oct3 %>%
    filter(!(Method %in% c("ridge", "ridge_smooth", "pms","PCA90_smooth_cov","PCA90_smooth_proj")))
  
  # Append the filtered results to the list
  all_aggregated_results_oct3[[signal_type]] <- aggregated_results_oct3_filtered
}

# Loop through each signal_type for the second base path (Oct 14)
for (signal_type in signal_types) {
  # Load the CSV file
  file_path <- paste0(base_path_oct14, signal_type, "_ridgepms.csv")
  results_oct14 <- read_csv(file_path)
  
  # Aggregate the results by Method and SignalStrength
  aggregated_results_oct14 <- results_oct14 %>%
    group_by(Method, SignalStrength) %>%
    summarise(
      Overlap_Median = median(Overlap, na.rm = TRUE),
      Overlap_SD = sd(Overlap, na.rm = TRUE),
    ...
    mutate(SignalType = signal_type)  # Add SignalType column
  #filter out pms_smooth
  aggregated_results_oct14_filtered <- aggregated_results_oct14 %>%
    filter(!(Method %in% c("pms_smooth")))
  # Append the filtered results to the list
  all_aggregated_results_oct14[[signal_type]] <- aggregated_results_oct14_filtered

}

# Concatenate all filtered and aggregated results from both Oct 3 and Oct 14 into one matrix
results_oct3 <- bind_rows(all_aggregated_results_oct3)
results_oct14 <- bind_rows(all_aggregated_results_oct14)

# Combine both results into a final dataframe
results <- bind_rows(results_oct3, results_oct14)
```
...
###Should also do a comparison of new methods
ridge old vs ridge new, pms vs pms0.6, holp vs holp
```{r}
# Load necessary packages
if (!require("pacman")) {install.packages("pacman"); library(pacman)}
p_load(dplyr, readr)
...
for (signal_type in signal_types) {
  ...
  # Filter in only the specified methods
  filtered_results_oct3 <- results_oct3 %>%
    filter(Method %in% c("ridge", "ridge_smooth", "pms", "holp"))
  
  # Rename the methods accordingly
  renamed_results_oct3 <- filtered_results_oct3 %>%
    mutate(Method = case_when(
      Method == "ridge" ~ "ridge_cv",
      Method == "ridge_smooth" ~ "ridge_cv_smooth",
      Method == "pms" ~ "pms0.9",
      Method == "holp" ~ "holp",
      TRUE ~ Method
    ))
  ...
  # Aggregate the results by Method and SignalStrength
  aggregated_results_oct3 <- renamed_results_oct3 %>%
    group_by(Method, SignalStrength) %>%
    summarise(
      Overlap_Median = median(Overlap, na.rm = TRUE),
      Overlap_SD = sd(Overlap, na.rm = TRUE),
  ...
  # Append the filtered results to the list
  all_aggregated_results_oct3[[signal_type]] <- aggregated_results_oct3
}
...
for (signal_type in signal_types) {
  ...
  # Rename the methods accordingly
  renamed_results_oct14 <- results_oct14 %>%
    mutate(Method = case_when(
      Method == "ridge" ~ "ridge_noCV",
      Method == "ridge_smooth" ~ "ridge_noCV_smooth",
      Method == "pms0.6" ~ "pms0.6",
      Method == "pms_smooth" ~ "pms0.6_smooth",
      Method == "holp_smooth" ~ "holp_smooth",
      TRUE ~ Method
    ))
  
  # Aggregate the results by Method and SignalStrength
  aggregated_results_oct14 <- renamed_results_oct14 %>%
    group_by(Method, SignalStrength) %>%
    summarise(
      Overlap_Median = median(Overlap, na.rm = TRUE),
      Overlap_SD = sd(Overlap, na.rm = TRUE),
  ...
  all_aggregated_results_oct14[[signal_type]] <- aggregated_results_oct14
}
...
custom_colors <- c(
  "pms0.6" = "#1b9e77",
  "pms0.6_smooth" = "#d95f02",
  "ridge_noCV" = "#7570b3",
  "ridge_noCV_smooth" = "#e7298a",
  "ridge_cv" = "#66a61e",
  "ridge_cv_smooth" = "#77868e",
  "pms0.9" = "#e6ab02",
  "holp" = "#a6761d",
  "holp_smooth" = "#466791"
)
...
# Define the signal types and the file path template
signal_types <- c("binmask")  # Add other signal types if needed
base_path <- "/well/nichols/users/qcv214/pms2/pile/sim_oct11_"

# Initialize an empty list to store the aggregated results for each signal_type
all_aggregated_results <- list()

# Loop through each signal_type for the first file (smooth.csv)
for (signal_type in signal_types) {
  # Load the CSV file
  file_path <- paste0(base_path, signal_type, "_smooth.csv")
  results_smooth <- read_csv(file_path)
  
  # Aggregate the results by Method and SignalStrength
  aggregated_results_smooth <- results_smooth %>%
    group_by(Method, SignalStrength) %>%
    summarise(
      Overlap_Median = median(Overlap, na.rm = TRUE),
      Overlap_SD = sd(Overlap, na.rm = TRUE),
  ...
  # Append to the list
  all_aggregated_results[[paste0(signal_type, "_smooth")]] <- aggregated_results_smooth
}

# Loop through each signal_type for the second file (smooth_pms.csv)
for (signal_type in signal_types) {
  # Load the CSV file
  file_path <- paste0(base_path, signal_type, "_smooth_pms.csv")
  results_smooth_pms <- read_csv(file_path)
  
  # Filter out any rows where the Method contains the prefix "pms_smooth"
  results_smooth_pms <- results_smooth_pms %>%
    filter(!str_detect(Method, "^pms_smooth"))
  
  # Aggregate the results by Method and SignalStrength
  aggregated_results_smooth_pms <- results_smooth_pms %>%
    group_by(Method, SignalStrength) %>%
    summarise(
      Overlap_Median = median(Overlap, na.rm = TRUE),
      Overlap_SD = sd(Overlap, na.rm = TRUE),
  ...
  # Append to the list
  all_aggregated_results[[paste0(signal_type, "_smooth_pms")]] <- aggregated_results_smooth_pms
}
...
# Define the signal types and the file path template
signal_types <- c("sparsemask")  # Add other signal types if needed
base_path <- "/well/nichols/users/qcv214/pms2/pile/sim_oct11_"
...
//profiling `sim_method_profiling.R`17934193

Running `sim_pms_grid.R` sim_oct16_pms_grid.csv  18162437
...
# Loop through each signal_type for the first file (smooth.csv)
  # Load the CSV file
  file_path <- paste0("/well/nichols/users/qcv214/pms2/pile/sim_oct16_pms_grid.csv")
  results_smooth <- read_csv(file_path)
  
  # Aggregate the results by Method and SignalStrength
  aggregated_results_smooth <- results_smooth %>%
    group_by(Method, SignalStrength, SignalType) %>%
    summarise(
      Overlap_Median = median(Overlap, na.rm = TRUE),
      Overlap_SD = sd(Overlap, na.rm = TRUE),
      FPR_Median = median(FPR, na.rm = TRUE),
      ...
      FDR_Median = median(FDR, na.rm = TRUE),
      FDR_SD = sd(FDR, na.rm = TRUE)
    )   # Add SignalType column
  


# Concatenate all aggregated results into one matrix
results <- bind_rows(aggregated_results_smooth)
```
```{r}
# Load the ggplot2 library
library(ggplot2)
...
```
pms 0.9 and 1 are always terrible
binmask: 0.4-0.6
sparse: it's clearly ordered.

##profiling
//`sim_method_profiling.R` added running raw HOLP: 18234682 ==> raw fitting of HOLP takes 4 seconds


#21 Oct

##Smooothing param
For the sake of simplicity and efficiency, we will try to gauge the smoothing param/correlation based on 1D vector of images instead of 3D (of course 3D has more precision). Two measures (measured by succeeding indices)
1. Laplacian smoothness (roughness penalty) - sum of diff squared
2. Variance of differences - var(diff)

##Acquiring signal

First, I need to obtain signals, for now I will only assume signals from the signal region that i fitted HOLP on
...
mask.vec <- c(fast_read_imgs_mask('/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz','/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz'))
mask.index <- (mask.vec %in% c(4,15))
mask.signal <- which(mask.index)[1:300] #let's try binmask here since we have pms to benchmark
mask.beta <- c(fast_read_imgs_mask('/well/nichols/users/qcv214/pms2/viz/sep25_age_HOLP_binmask_fit.nii.gz','/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz'))

#signal-area only
# signals <- mask.beta[mask.signal]
#non-signal area as well
mask.beta[!mask.index] <- 0
signals <- mask.beta

normalize_smoothness <- function(smoothness, raw_signal) {
  vector_length <- length(raw_signal)
  min_val <- min(raw_signal)
  max_val <- max(raw_signal)
  # The maximal roughness would occur when every adjacent element alternates between min_val and max_val.
  max_roughness = (vector_length - 1) * (max_val - min_val)^2
  # Minimal roughness is 0 when the vector is constant.
  min_roughness = 0
  
  # Normalize the smoothness between 0 and 1
  normalized_smoothness = (smoothness - min_roughness) / (max_roughness - min_roughness)
  return(1 - normalized_smoothness)  # Higher values indicate more smoothness
}

# Compute Laplacian smoothness (sum of squared differences)
laplacian_smoothness <- sum(diff(signals)^2)

#normalise
smoothness_score <- normalize_smoothness(laplacian_smoothness, signals)

print(smoothness_score)  # Smoothness score between 0 and 1
```
0.977 for 1867 voxels of signals with non-zero
0.997 for all voxels where 1867 of those are non-zero
0.964 for binmask

I think it makes sense to use the first case since at non-signals area, we shouldn't be caring about smoothing anyway. 
But then it doesn't make sense if signals are not defined to be contiguous right? So need to make use of the whole image even though it's 0??

what's interesting is that signals seem to be normally dist around 0 (via plot(density(signals))), so this result of smoothness is similar to drawing samples from normal dist...

Maybe we can use sd or var to make an estimate? but these don't take into account of smoothness. Then maybe var of diff or sd of diff??

##plotting dist
```{r}
#signals
plot(density(signals))

plot(density(diff(signals))) #this is still gaussian. => so maybe use sd?
```
sd of diff is only 0.237


###variance  method
```{r}
normalize_variance_diff <- function(variance_diff, data) {
  total_variance <- var(data)
  normalized_smoothness <- 1 - (variance_diff / total_variance)
  return(normalized_smoothness)
}

# Compute variance of differences
variance_diff <- var(diff(signals))

# Normalize by total variance
smoothness_score <- normalize_variance_diff(variance_diff, signals)
print(smoothness_score)
``` 
This is negative - not really usable?

Note that if var(diff(signals)) > var(signals) then signals are not smooth? it means consecutive signals are not smooth wrt overall variabiliy 
```{r}
var(diff(signals))- var(signals)
```


#23 Oct

We can estimate the spatial correlation by computing lag-1 correlation in each direction x,y,z e.g. by shifting the image.
```{r}
##load beta
library(PMS)
#load beta
...
compute_lag1_correlation <- function(img, axis) {
  ...
  valid_mask <- shifted_img != 0  # Exclude any potential zero-padded regions ==> #this part break sparse mask in x direction
  img_vector <- img[valid_mask] #basically this becomes empty due to no overlapping in sparse mask.
...
cat("Lag-1 correlation in x-direction: ", lag1_corr_x, "\n")
cat("Lag-1 correlation in y-direction: ", lag1_corr_y, "\n")
cat("Lag-1 correlation in z-direction: ", lag1_corr_z, "\n")
cat("Lag-1 correlation in geom mean: ", (abs(lag1_corr_x) * abs(lag1_corr_y) * abs(lag1_corr_z))^(1/3), "\n") #this does not make sense

cat("Lag-1 correlation in SUM-direction: ", lag1_corr_x + lag1_corr_y + lag1_corr_z, "\n")
cat("Lag-1 correlation in MEAN-direction: ", (lag1_corr_x + lag1_corr_y + lag1_corr_z)/3, "\n")

```
What make sense the most if sum of these directional corr, equal to 0.45 ==> this is most intuitive but I need to verify this maybe with another case.
Summing can make it exceed 1!!
...
mask.vec <- c(fast_read_imgs_mask('/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz','/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz'))
mask.index <- (mask.vec %in% c(4,15))
mask.signal <- which(mask.index)[seq(1,1500,5)]#let's try binmask here since we have pms to benchmark
mask.beta <- c(fast_read_imgs_mask('/well/nichols/users/qcv214/pms2/viz/sep25_age_HOLP_binmask_fit.nii.gz','/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz'))
...
# Compute lag-1 correlation in each direction (x, y, z)
lag1_corr_x <- compute_lag1_correlation(img, "x") #got sd = 0???
lag1_corr_y <- compute_lag1_correlation(img, "y")
lag1_corr_z <- compute_lag1_correlation(img, "z")
...
cat("Lag-1 correlation in SUM-direction: ", lag1_corr_x + lag1_corr_y + lag1_corr_z, "\n")
```
Take NA as "no smoothness"


L2 norm of diff => this measures roughness
```{r}
# Function to compute normalized smoothness (opposite of roughness) based on lag-1 differences along a specified axis
compute_normalized_smoothness <- function(img, axis) {
  # Get dimensions of the 3D image
  dims <- dim(img)
  
  # Compute lag-1 differences along the specified axis
  if (axis == "x") {
    diff_img <- img[2:dims[1], , ] - img[1:(dims[1]-1), , ]
  } else if (axis == "y") {
    diff_img <- img[, 2:dims[2], ] - img[, 1:(dims[2]-1), ]
  } else if (axis == "z") {
    diff_img <- img[, , 2:dims[3]] - img[, , 1:(dims[3]-1)]
  }

  # Compute the L2 norm of the differences (roughness measure)
  roughness <- sqrt(sum(diff_img^2, na.rm = TRUE))
  
  # Normalize the smoothness measure: 1 for no roughness, approaching 0 as roughness increases
  max_possible_roughness <- sqrt(length(diff_img))  # Maximum roughness if each difference is maximally distinct ==> this is way too big due to 91x109x91
  print(max(abs(diff_img)))
  smoothness <- 1 - (roughness / max_possible_roughness)
  smoothness <- max(0, min(smoothness, 1))  # Ensure smoothness is within [0, 1]
...
# Compute normalized smoothness in each direction (x, y, z)
smoothness_x <- compute_normalized_smoothness(img, "x")
smoothness_y <- compute_normalized_smoothness(img, "y")
smoothness_z <- compute_normalized_smoothness(img, "z")

# Output the smoothness values
cat("Normalized smoothness in x-direction: ", smoothness_x, "\n")
cat("Normalized smoothness in y-direction: ", smoothness_y, "\n")
cat("Normalized smoothness in z-direction: ", smoothness_z, "\n")
```


#25 Oct

To estimate sigma, we can try the following:
1. Do HOLP and lag-1 correlation like above
2. Do HOLP and compute lag-1 L2 norm for smoothness (need to normalised) => this is too difficult to formulate into usable metric... it's only good as a relative metric
3. Fit voxel-wise correlation, then perform 1 or 2.

Jian said both the correlation and the L2norm methods for finding correlation/smoothness sound good too.

The other correlation we can do is we can voxel-wise fit correlation of y against x to get a (spatially-varying) coefficients correlation image, which we can use instead of fitted beta/holp too.

Created `gen_age_cor_binmask.R` to save `pms2/viz/oct25_age_cor_binmask_fit` // 19388839
...
mask.index <- (mask.vec %in% c(4,15))
mask.signal <- which(mask.index)[1:300]#let's try binmask here since we have pms to benchmark
mask.beta <- c(fast_read_imgs_mask('/well/nichols/users/qcv214/pms2/viz/oct25_age_cor_binmask_fit.nii.gz','/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz'))
...
cat("Lag-1 correlation in z-direction: ", lag1_corr_z, "\n")
cat("Lag-1 correlation in SUM-direction: ", lag1_corr_x + lag1_corr_y + lag1_corr_z, "\n")
cat("Lag-1 correlation in geom mean: ", (abs(lag1_corr_x) * abs(lag1_corr_y) * abs(lag1_corr_z))^(1/3), "\n") #this looks particularly decent. 
cat("Lag-1 correlation in power flatness mean: ", (abs(lag1_corr_x) * abs(lag1_corr_y) * abs(lag1_corr_z))^(1/3)/((abs(lag1_corr_x) + abs(lag1_corr_y) + abs(lag1_corr_z))/3), "\n") #this does not make sense. 

cat("Lag-1 correlation in flatness: ", (abs(lag1_corr_x) * abs(lag1_corr_y) * abs(lag1_corr_z))/(abs(lag1_corr_x) + abs(lag1_corr_y) + abs(lag1_corr_z)), "\n")
cat("Lag-1 correlation in mean: ", (abs(lag1_corr_x) + abs(lag1_corr_y) + abs(lag1_corr_z))/3, "\n") #this looks decent, maybe most decent. 

```
So suppose corr is 0.4567, the corresponding smoothness should be 1/sqrt(-2*log(0.4567))
...
mask.vec <- c(fast_read_imgs_mask('/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz','/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz'))
mask.index <- (mask.vec %in% c(4,15))
mask.signal <- which(mask.index)[c(1:150, 1001:1150)]#let's try binmask here since we have pms to benchmark
mask.beta <- c(fast_read_imgs_mask('/well/nichols/users/qcv214/pms2/viz/oct25_age_cor_binmask_fit.nii.gz','/well/nichols/users/qcv214/pms2/sub150_centre_mask.nii.gz'))
...
cat("Lag-1 correlation in flatness: ", (abs(lag1_corr_x) * abs(lag1_corr_y) * abs(lag1_corr_z))/(abs(lag1_corr_x) + abs(lag1_corr_y) + abs(lag1_corr_z)), "\n")
cat("Lag-1 correlation in mean: ", (abs(lag1_corr_x) + abs(lag1_corr_y) + abs(lag1_corr_z))/3, "\n") #this looks decent, maybe most decent. 
cat("Lag-1 correlation in mean no abs: ", ((lag1_corr_x) + (lag1_corr_y) + (lag1_corr_z))/3, "\n") #this looks decent, maybe most decent. 

```
So suppose corr is 0.3767469, the corresponding smoothness should be 1/sqrt(-2*log(0.3767469)), 0.7156813
if corr is 0.1933201 then it's 


Apply it to smoonthness algo to see improvement... in ridge smooth, etc
Running
//sim_oct28_binmask_smooth.csv 19564054
```{r}
# Load necessary packages
if (!require("pacman")) {install.packages("pacman"); library(pacman)}
p_load(dplyr, readr)
...
# Define the signal types and the file path templates
signal_types <- c("binmask")
base_paths <- c("/well/nichols/users/qcv214/pms2/pile/sim_oct11_", 
                "/well/nichols/users/qcv214/pms2/pile/sim_oct28_")

# Initialize an empty list to store the aggregated results for each file and signal type
all_aggregated_results <- list()

# Loop through each base path and each signal type
for (base_path in base_paths) {
  for (signal_type in signal_types) {
    ...
    aggregated_results <- results %>%
      ...
      mutate(SignalType = signal_type, SourceFile = basename(file_path))  # Add SignalType and SourceFile columns
    
    # Append to the list
    all_aggregated_results[[paste0(signal_type, "_", basename(base_path))]] <- aggregated_results
  }
}

# Concatenate all aggregated results into one data frame
results <- bind_rows(all_aggregated_results)
...
# Smoothing parameters
smoothing_params <- c(0.25, 0.5, 0.798733989470034, 1, 2)
...
```
Ineed, this is where SPCA proj achieves highest.

Need to put to test on real data
Created
[half failed, didin't define shrinkage param] `co_pm_cv_full.R` all saved under `oct29_pm` 19765374 19883735(for smooth) ==> so many modification... 
[half failed, didin't define shrinkage param] `age_cv_full.R` all saved under `oct29_age` 19787742 19883734(for smooth)
The latter each ran for 4 hours and got OOM at spca proj

  - changed dcv to cv for ridge
  - changed theta.range to smaller
  - changed num.vox.vec to smaller
  - changed simple cv to using 5000 variables
  - tune smoothing param via voxel-wise corr
  - changed shinkrage param to 0.3 (from 0.01)


//`co_pm_cv_full_smooth_pca.R` 19900282`co_pm_cv_full_smooth_spca.R` 19900333
//`age_cv_full_smooth_pca.R` 19900398`age_cv_full_smooth_spca.R` 19900423

correlation: 0.5878352, ie sigma = 0.97 (extremely close to 1)
...
# Read data
res <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_pm_pms_full.csv")
res3 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_pm_nbpms_full.csv")
res3 <-  cbind(res3, matrix(data=NA, nrow=nrow(res3), ncol = (length(res) - length(res3))))
colnames(res3) <- colnames(res)
res4 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_pm_pms90_smooth_shrink_pca.csv")
res4 <-  cbind(res4, matrix(data=NA, nrow=nrow(res4), ncol = (length(res) - length(res4))))
colnames(res4) <- colnames(res)
res5 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_pm_pms90_smooth_shrink_spca.csv")
res5 <-  cbind(res5, matrix(data=NA, nrow=nrow(res5), ncol = (length(res) - length(res5))))
colnames(res5) <- colnames(res)
...
# First plot
resdat <- as.data.frame(rbind(res[1:3,], res3[1:2, ], res4[1:4, ], res5[1:3, ]))
num.vox.vec <- (1:25)*400
colnames(resdat) <- num.vox.vec
resdat$row_id <- c('PCA 90','SPCA 90','Ridge','PMS','PMS_holp','Smooth_Ridge','Smooth_PCA'
                   ,'SmoothPCA_Proj','SmoothPCA_Cov','Smooth_SPCA','SmoothSPCA_Proj','SmoothSPCA_Cov')
...
# Second plot
resdat <- as.data.frame(rbind(res[4:6,], res3[3:4, ], res4[5:8, ], res5[4:6, ]))
colnames(resdat) <- num.vox.vec
resdat$row_id <- c('PCA 90','SPCA 90','Ridge','PMS','PMS_holp','Smooth_Ridge','Smooth_PCA','Smooth_SPCA'
                   ,'SmoothPCA_Proj','SmoothSPCA_Proj','SmoothPCA_Cov','SmoothSPCA_Cov')
...
ggplot(resdat_long, aes(x = as.numeric(column), y = value, group = row_id, color = as.factor(row_id))) +
  ...
  labs(title = "Test RMSE", x = "No. Variables", y = "RMSE", color = "Method")
```
This divergence looks very scary. It basically means that this method keep picking non-relevant voxels.
...
# Read data
res <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_pms_full.csv")
res3 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_nbpms_full.csv")
res3 <-  cbind(res3, matrix(data=NA, nrow=nrow(res3), ncol = (length(res) - length(res3))))
colnames(res3) <- colnames(res)
res4 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_pms90_smooth_shrink_pca.csv")
res4 <-  cbind(res4, matrix(data=NA, nrow=nrow(res4), ncol = (length(res) - length(res4))))
colnames(res4) <- colnames(res)
res5 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_pms90_smooth_shrink_spca.csv")
res5 <-  cbind(res5, matrix(data=NA, nrow=nrow(res5), ncol = (length(res) - length(res5))))
colnames(res5) <- colnames(res)
...
ggplot(resdat_long, aes(x = as.numeric(column), y = value, group = row_id, color = as.factor(row_id))) +
  ...
  labs(title = "Test RMSE", x = "No. Variables", y = "RMSE", color = "Method")
```
Smoothing make results worse for SPCA.


Actually if modelling pair matching isn't good. Then I can totally ignore it.


I should also compare the result of all smoothing new and old.
Note that with PMS too
###Comparing sigma = 1 and computed 0.97
these two numbers are crazy close, I dont think they are comparable


Here are older results, note that here ridge and SPCA are double cross-validated
```{r}
# Load necessary libraries
library(ggplot2)
library(tidyr)
...
# Second plot
resdat <- as.data.frame(rbind(res[4:6,], res2[c(4:6),], res3[3:4, ], res4[7:10, ]))
colnames(resdat) <- num.vox.vec
resdat$row_id <- c('PCA 90','SPCA 90','Ridge', 'Smooth_PCA 90','Smooth_SPCA 90','Smooth_Ridge','PMS','PMS_holp'
                   ,'SmoothPCA_Proj','SmoothSPCA_Proj','SmoothPCA_Cov','SmoothSPCA_Cov')
# Reshape the data to long format
...
##simulation
//sim_oct28_dbmask_smooth.csv  19926030 => using voxel-wise smoothness


###Turning binmask result into table
```{r}
# Load necessary packages
if (!require("pacman")) {install.packages("pacman"); library(pacman)}
p_load(dplyr, readr)
...
# Define the signal types and the file path templates
signal_types <- c("binmask")  # Add additional signal types as needed
base_paths <- c(
  #"/well/nichols/users/qcv214/pms2/pile/sim_oct11_", 
  "/well/nichols/users/qcv214/pms2/pile/sim_oct28_",
  "/well/nichols/users/qcv214/pms2/pile/sim_oct3_"
)
...
for (base_path in base_paths) {
  for (signal_type in signal_types) {
    # Define file suffix based on the path type
    if (grepl("oct3", base_path)) {
      file_suffix <- ".csv"
    } else {
      file_suffix <- "_smooth.csv"
    }
    
    # Load the CSV file
    file_path <- paste0(base_path, signal_type, file_suffix)
...
print(results)
```
Something is currently wrong with oct3... somehow it's empty right now. need to re-run
//`sim_oct3_binmask.csv`. 19908905 ==> problem is this still does glmnet.
...
# Pivot the table to have "SignalStrength" as columns with "Overlap_Median" as the values
# results_pivoted <- results %>%
#   select(Method, SignalType, SourceFile, SignalStrength, Overlap_Median) %>%
#   pivot_wider(names_from = SignalStrength, values_from = Overlap_Median, 
#               names_prefix = "Overlap_Median_")
results_pivoted <- results %>%
  select(Method, SignalStrength, Overlap_Median) %>%
  pivot_wider(names_from = SignalStrength, values_from = Overlap_Median, 
              names_prefix = "Overlap_Median_") %>%
  mutate(across(starts_with("Overlap_Median_"), ~ round(., 1)))

# View the transformed table
# print(results_pivoted)
...
file_path <- "/well/nichols/users/qcv214/pms2/tables/results_binmask_31oct.csv"

# Export the table as a CSV file
write.csv(results_pivoted, file = file_path, row.names = FALSE)
...
signal_types <- c("dbmask")#, "dbmask", "sparsemask")
# signal_types <- c("binmask")
base_path <- "/well/nichols/users/qcv214/pms2/pile/sim_oct28_"
...
```
it's worse than smoothing at 0.25 and 0.5
Maybe I need to tune pms here as well to look. 


##Fixing smoothness for real data
//`age_cv_full_pca_fixsmooth.R` oct29_age_pms90_smooth0.5 and oct29_age_pms90_smooth2  20196193
//`age_cv_full_spca_fixsmooth.R` 20196177 (0.5) and 20431538 (2) have to run spearately due to OOM
`age_cv_full_checkcorr.R` ==> checking the compute correlation (mean without abs) to see if i needa re-run 20207567
So with abs and WITHOUT ABS(), it's 0.7285967 , its 0.7285967, this means sigma = 1.256625


##Simulation
db mask
//sim_oct31_dbmask.csv 20301466 => fix ridge and spca
//sim_oct31_dbmask_smooth.csv  20302470


###db mask with smoothing
```{r}
# Load necessary packages
if (!require("pacman")) {install.packages("pacman"); library(pacman)}
p_load(dplyr, readr, ggplot2)
...
# Define the signal types and the file path template
signal_types <- c("dbmask")
base_path <- "/well/nichols/users/qcv214/pms2/pile/sim_oct31_"
...
# Smoothing parameters
smoothing_params <- c(0.25, 0.5, 0.5515848, 0.7156813, 1, 2)
...
for (method in names(base_colors)) {
  for (smooth_val in smoothing_params) {
    # Correct naming convention for method with smoothing parameter
    method_with_suffix <- gsub("_smooth", paste0("_smooth", smooth_val), method, fixed = TRUE)
    custom_colors[[method_with_suffix]] <- base_colors[[method]]
  }
}
...
for (smooth_val in smoothing_params) {
  ...
  for (signal_type in signal_types) {
    # Filter the results for the current SignalType and exact smoothing parameter in Method
    pattern <- paste0("_smooth", smooth_val, "(?![0-9])")  # Match exact segment without extra digits after smooth_val
    signal_data <- subset(results, SignalType == signal_type & grepl(pattern, Method, perl = TRUE))
...
# resdat$row_id <- c('PCA 90','SPCA 90','Ridge','PMS','PMS_holp','Smooth_Ridge','Smooth_PCA'
#                    ,'SmoothPCA_Proj','SmoothPCA_Cov','Smooth_SPCA','SmoothSPCA_Proj','SmoothSPCA_Cov')
resdat$row_id <- c('Ridge','PCA 90','SPCA 90','PMS','PMS_holp','Smooth_Ridge','Smooth_PCA'
                   ,'SmoothPCA_Proj','SmoothPCA_Cov','Smooth_SPCA','SmoothSPCA_Proj','SmoothSPCA_Cov')
# Reshape the data to long format
resdat_long <- pivot_longer(resdat, cols = -row_id, names_to = "column", values_to = "value")
...
ggplot(resdat_long, aes(x = as.numeric(column), y = value, group = row_id, color = as.factor(row_id))) +
  ...
  labs(title = "Test RMSE", x = "No. Variables", y = "RMSE", color = "Method")
```
Now that we fixed it, SPCA is the worst and PCA is the best!!!
...
# Read data
res <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_pms90_smooth_shrink_pca.csv")
res3 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_pms90_smooth_shrink_spca.csv")
res3 <-  cbind(res3, matrix(data=NA, nrow=nrow(res3), ncol = (length(res) - length(res3))))
colnames(res3) <- colnames(res)
res4 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_pms90_smooth0.5_shrink_pca.csv")
res4 <-  cbind(res4, matrix(data=NA, nrow=nrow(res4), ncol = (length(res) - length(res4))))
colnames(res4) <- colnames(res)
res5 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_pms90_smooth0.5_shrink_spca.csv")
res5 <-  cbind(res5, matrix(data=NA, nrow=nrow(res5), ncol = (length(res) - length(res5))))
colnames(res5) <- colnames(res)
res6 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_pms90_smooth2_shrink_spca.csv")
res6 <-  cbind(res6, matrix(data=NA, nrow=nrow(res6), ncol = (length(res) - length(res6))))
colnames(res6) <- colnames(res)
res7 <- read.csv("/well/nichols/users/qcv214/pms2/pile/oct29_age_pms90_smooth2_shrink_spca.csv")
res7 <-  cbind(res7, matrix(data=NA, nrow=nrow(res7), ncol = (length(res) - length(res7))))
colnames(res7) <- colnames(res)
...
custom_colors <- c(
  ...
  "Smooth_Ridge" = "#FFFF33",         # Yellow
  "SmoothPCA_Proj" = "#8DD3C7",       # Light teal
  "SmoothSPCA_Proj" = "#66C2A5",      # Medium teal
  "SmoothPCA_Cov" = "#B3DE69",        # Light green
  "SmoothSPCA_Cov" = "#FB8072",       # Salmon pink
  
  # Smooth 0.5
  "Smooth0.5_PCA" = "#C51B7D",        # Darker purple
  "Smooth0.5_SPCA" = "#E41A1C",       # Red-orange
  "Smooth0.5_Ridge" = "#FFD92F",      # Golden yellow
  "Smooth0.5PCA_Proj" = "#A6D853",    # Light green
  "Smooth0.5SPCA_Proj" = "#66A61E",   # Green
  "Smooth0.5PCA_Cov" = "#FFB3AB",     # Pale pink
  "Smooth0.5SPCA_Cov" = "#F781BF",    # Pink
  
  # Smooth 2
  "Smooth2_PCA" = "#756BB1",          # Medium purple
  "Smooth2_SPCA" = "#D95F02",         # Dark orange
  "Smooth2_Ridge" = "#BC80BD",        # Soft lavender
  "Smooth2PCA_Proj" = "#B3E2CD",      # Mint green
  "Smooth2SPCA_Proj" = "#5AB4AC",     # Bluish teal
  "Smooth2PCA_Cov" = "#FDCDAC",       # Pale salmon
  "Smooth2SPCA_Cov" = "#E78AC3"       # Light pink
)

# First plot
resdat <- as.data.frame(rbind(res[1:4,], res3[1:3, ], res4[1:4, ], res5[1:3, ], res6[1:4, ], res7[1:3, ]))
num.vox.vec <- (1:25)*400
colnames(resdat) <- num.vox.vec
# resdat$row_id <- c('PCA 90','SPCA 90','Ridge','PMS','PMS_holp','Smooth_Ridge','Smooth_PCA'
#                    ,'SmoothPCA_Proj','SmoothPCA_Cov','Smooth_SPCA','SmoothSPCA_Proj','SmoothSPCA_Cov')
resdat$row_id <- c('Smooth_Ridge','Smooth_PCA','SmoothPCA_Proj','SmoothPCA_Cov','Smooth_SPCA','SmoothSPCA_Proj','SmoothSPCA_Cov',
                   'Smooth0.5_Ridge','Smooth0.5_PCA','Smooth0.5PCA_Proj','Smooth0.5PCA_Cov','Smooth0.5_SPCA','Smooth0.5SPCA_Proj','Smooth0.5SPCA_Cov',
                   'Smooth2_Ridge','Smooth2_PCA','Smooth2PCA_Proj','Smooth2PCA_Cov','Smooth2_SPCA','Smooth2SPCA_Proj','Smooth2SPCA_Cov')
# Reshape the data to long format
resdat_long <- pivot_longer(resdat, cols = -row_id, names_to = "column", values_to = "value")
...
# Second plot
resdat <- as.data.frame(rbind(res[5:8,], res3[4:6, ], res4[5:8, ], res5[4:6, ], res6[5:8, ], res7[4:6, ]))
...
ggplot(resdat_long, aes(x = as.numeric(column), y = value, group = row_id, color = as.factor(row_id))) +
  ...
  labs(title = "Test RMSE", x = "No. Variables", y = "RMSE", color = "Method")
```
I think ridge is in the wrong place with PCA
